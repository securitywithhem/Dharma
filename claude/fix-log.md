...
| 3.4b Org delete does not cascade | TODO | Found by `acf75de`, worked around in the cleanup script only. Real tenant offboarding still fails. |
...
1. **WAVE 0.1–0.5** — the whole VAPT authorization wave. Nothing exists.
2. **3.4b** — Organization delete cascade.
3. **4.2b** — Cross-Walk matrix auto-population, re-assessed post-embedding-fix.
