...
async function cleanInPlaceArtifacts() {
  ...
  const [schedules, endpoints, apiKeys] = await Promise.all([
    prisma.reportSchedule.deleteMany({ where: { title: { startsWith: "E2E ", mode: "insensitive" } } }),
  ...
  if (!APPLY) {
    ...
    console.log(
      `\nWOULD ALSO CLEAR in-place E2E rows — ${schedules} report schedules, ` +
        `${endpoints} endpoints, ${apiKeys} API keys.`,
    );
